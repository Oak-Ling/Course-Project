...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		//initialize body, width and height simultaneously
		width = 0;
		height = 0;
		...
		//initialize skirt
		skirt = new int[width];
		Arrays.fill(skirt, height); //set the default value
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		int moveX = 0; //check whether need to move the x axis
		int moveY = 0; //check whether need to move the y axis
		for (int i = 0; i < body.length; i++) {
			TPoint curr = new TPoint(-body[i].y, body[i].x);
			if (curr.x < 0) moveX++;
			if (curr.y < 0) moveY++;
			newBody[i] = curr;
		}
		
		/* move the Tetris to the first quadrant if needed*/
		if (moveX > 0) {
			for (TPoint point : newBody) {
				point.x += height - 1;
			}
		}
		if (moveY > 0) {
		...
		Piece rotatedPiece = new Piece(newBody);
		return rotatedPiece; 
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		if (body.length != other.body.length) 
			return false;
		// compare item in the body
		int count = 0;
		for (int i = 0; i < body.length; i++) { //iterate other.body 
			for (int j = 0; j < body.length; j++) { //iterate body
				if (other.body[i].equals(body[j])) {
					count++;
				}
			}
			if (count == 0) return false; //for the first point, don't find the match
			if (count == 1) count = 0; //find one match in the body
		}
		return true;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while (true) {
			...
			if (root.equals(curr.next)) break;
			curr = curr.next;
		}
		curr.next = root;
		return root; 
