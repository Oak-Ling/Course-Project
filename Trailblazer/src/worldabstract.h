...
public:
    ...
    static const std::string COLOR_ALT_PATH_STR;
    ...
    /**
     * Returns true if this world currently has a non-null start and end
     * selected vertex.
     */
    virtual bool hasSelectedVertexes() const;

    /**
     * Given a path, returns the combined cost of all edges in that path.
     * Throws an error if path is not valid or any vertex/edge not found in graph.
     */
    virtual double pathComputeCost(Vector<Vertex*>& path) const;
     ...
     */
    virtual void drawPath(Vector<Vertex*>& path, Color color = RED) = 0;
    ...
    /**
     * Since World extends Observer, the following pure virtual member
...
protected:
    ...
    // member variables
    GWindow* _gwnd;                    // graphical window for drawing graph
    double _windowWidth;               // width/height to use for display
    double _windowHeight;
    GDimension _preferredSize;         // size graph would like to be
    BasicGraph* _graph;                // the graph itself
    Vertex* _selectedStart;            // currently selected start/end vertices
    Vertex* _selectedEnd;              //   from clicks (nullptr if none)
    // Vertex* _hover;                 // mouse-over hover vertex (nullptr if none)
    Vector<GLine*> _highlightedPath;   // highlighted path lines (empty if none)
