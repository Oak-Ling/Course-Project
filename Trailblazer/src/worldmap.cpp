...
const int WorldMap::LABEL_FONT_SIZE = 10;
...
void WorldMap::drawVertexCircle(Vertex* v, std::string color, bool fill) {
    ...
    oval.setFilled(fill);
    if (v == _selectedStart || v == _selectedEnd) {
...
void WorldMap::draw() {
    ...
    _gwnd->setColor(0x0);
    _gwnd->fillRect(0, 0, _preferredSize.getWidth(), _preferredSize.getHeight());
    
    if (_backgroundImage) {
        _gwnd->draw(_backgroundImage, WINDOW_MARGIN, WINDOW_MARGIN);
    }
    ...
    if (isLargeWorld()) {
        _gwnd->repaint();
        _gwnd->setRepaintImmediately(true);
    }
}

void WorldMap::drawPath(Vector<Vertex*>& path, Color color) {
    ...
    for (int i = 1; i < path.size(); i++) {
        ...
        Point* p1 = static_cast<Point*>(path[i - 1]->extraData);
        Point* p2 = static_cast<Point*>(path[i]->extraData);
...
Vertex* WorldMap::getVertex(double x, double y) const {
    for (Vertex* v : *_graph) {
        Point* p = static_cast<Point*>(v->extraData);
        GOval oval(WINDOW_MARGIN + p->getX() - getVertexRadius(),
                   WINDOW_MARGIN + p->getY() - getVertexRadius(),
                   2 * getVertexRadius(), 2 * getVertexRadius());
...
int WorldMap::getVertexRadius() const {
    return static_cast<int>(isLargeWorld() ? VERTEX_RADIUS_LARGE_MAP : VERTEX_RADIUS);
}
...
double WorldMap::heuristic(Vertex* v1, Vertex* v2) {
    ...
    if (_heuristicEnabled) {
        Point* p1 = static_cast<Point*>(v1->extraData);
        Point* p2 = static_cast<Point*>(v2->extraData);
...
bool WorldMap::read(std::istream& input) {
    ...
    _backgroundImage = new GImage(imageFile);
    ...
    if (line == "FLAGS" || line == "VERTEXES" || line == "EDGES") {
        ...
        while (getline(input, line)) {
            ...
            if (flag == "heuristic") {
                _heuristicEnabled = stringToBool(value);
    ...
    while (getNonEmptyLine(input, line)) {
        ...
        Vertex* v = _graph->addVertex(name);
        v->extraData = static_cast<void*>(new Point(vertexX, vertexY));
        v->addObserver(this);
    }
    ...
    while (getNonEmptyLine(input, line)) {
        ...
        if (!_graph->containsVertex(name1)) {
        ...
        if (!_graph->containsVertex(name2)) {
            std::cerr << "Invalid input file; when reading edge between \""
                      << name1 << "\" and \"" << name2
                      << "\", graph does not contain a vertex named \""
                      << name2 << "\"" << std::endl;
        ...
        if (ignoreWeight) {
            ...
            weight = heuristic(_graph->getVertex(name1), _graph->getVertex(name2));
        }
        
        _graph->addEdge(name1, name2, weight, /* directed */ directed);
    }
...
void WorldMap::update(Observable<WorldEvent>* obs, const WorldEvent& /*arg*/) {
    Vertex* v = static_cast<Vertex*>(obs);
    int r, g, b;
