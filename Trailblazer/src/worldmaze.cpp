...
void WorldMaze::createRandomMaze(WorldSize size) {
    ...
    Set<Edge*> edgeSet = _graph->getEdgeSet();
    ...
    for (Edge* edge : mst) {
        ...
        } else if (!_graph->containsEdge(edge->start, edge->finish)) {
    ...
    for (int i = 0; i < edgeEndpoints.size() - 1; i += 2) {
        ...
        _graph->addEdge(start, finish);
        _graph->addEdge(finish, start);
    }
    ...
    pxPerWidth = _windowWidth / worldSize;
    pxPerHeight = _windowHeight / worldSize;
        ...
        for (int col = 0; col < rowsCols; col++) {
            ...
            for (int dr = -1; dr <= 1; dr++) {
                ...
                for (int dc = -1; dc <= 1; dc++) {
                    ...
                    if (_graph->containsEdge(name, neighborName)) {
